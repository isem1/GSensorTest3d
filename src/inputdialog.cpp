...
void InputDialog::setSerialPort(VirtualPort* pComPort) noexcept {
    m_pSerialPort = pComPort;
}

QString InputDialog::getComPort() const noexcept {
    return m_pTxtComPort->currentText();
}

QString InputDialog::getBaudRate() const noexcept {
    return m_pTxtBaudRate->currentText();
}

void InputDialog::initializeWidget() noexcept {
...
void InputDialog::addElements() noexcept {
    ...
    connect(cmdOk, &QPushButton::clicked, this, &InputDialog::dialogClose);
    connect(cmdCancel, &QPushButton::clicked, this, &InputDialog::reject);
}

void InputDialog::fillComboBoxes() noexcept {
    // Available COM Ports
    ...
    // Standard baud rates
    QStringList baudList = { "1200", "2400", "4800",
...
void InputDialog::dialogClose() {
    if (m_pSerialPort != nullptr)
        m_pSerialPort->openComPort(getComPort(), getBaudRate());
