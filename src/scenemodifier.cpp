...
void SceneModifier::setModelLoader(ModelLoader* modelLoader) {
    m_pModelLoader = modelLoader;
}

void SceneModifier::enableCuboid(bool enabled) noexcept {
    m_modelEntity->setEnabled(enabled);
}

void SceneModifier::setCuboidRotationX(float angle) noexcept {
    m_transformData->setRotationX(angle);
}

void SceneModifier::setCuboidRotationY(float angle) noexcept {
    m_transformData->setRotationZ(angle);
}

float SceneModifier::getCuboidRotationX() noexcept {
    return m_transformData->rotationX();
}

float SceneModifier::getCuboidRotationY() noexcept {
    return m_transformData->rotationZ();
}

void SceneModifier::onModelPathChanged() {
    m_objModel = new Qt3DRender::QMesh();
    m_objModel->setSource(QUrl::fromLocalFile(
